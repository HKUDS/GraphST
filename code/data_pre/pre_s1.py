...
# address = census_block["ADDRESS"].values.tolist()
# address_split = [item.split(",")[0] for item in address]
# add_pos = {}
# i= 0
# # tmp_dict={}
# for ad in address_split:
#     start_t = time.time()
#     i+=1
#     geolocater = Nominatim(user_agent='demo_of_gnss_help')
#     # print("ad:", ad)
#     try:
#         if ad not in add_pos.keys():
#             location = geolocater.geocode(ad)
...
#                 # add_pos.append([location.latitude, location.longitude])
#                 # if ad not in tmp_dict.keys():
#                 # tmp_dict[ad] = []
#                 add_pos[ad] = [location.latitude, location.longitude]
