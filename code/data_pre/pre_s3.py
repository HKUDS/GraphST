...
region_traffic = load_data("../data/NY_traffic.pickle")
# region_ = {}
...
# print(region_back.keys())
# print("@@:", len(region))
# print("%%:", len(region_back))
# printlnn
...
for key,value in result.to_dict().items():
    ...
    pair = ('r_{}_{}'.format(key[0], int(key[-1])),'r_{}_{}'.format(key[1], int(key[-1]+1)), {"weight":value, "date":int(key[-1]), "start":'r_{}_{}'.format(key[0], int(key[-1])), "end":'r_{}_{}'.format(key[1], int(key[-1]+1))})
...
file=open(r"../data/flow_graph.pickle","wb")
pickle.dump(G_flow,file) #storing_list
file.close()

file=open(r"../data/spatial_graph.pickle","wb")
