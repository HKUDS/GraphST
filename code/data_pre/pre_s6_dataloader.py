...
resolution = 500
node_features = load_data("../data/reg_com_poi_cat_spatial.pickle")
...
def nx_to_graph_data_obj(g):
    ...
    x = torch.tensor([node_features[item].tolist() for item in n_nodes])
    # print("x:",x.size())
    # print(x_.size())
    # printnln()
    file=open(r"../data/nodes_{}.pickle".format(resolution),"wb")
...
#     return data
hy_graph = load_data("../data/hy_8.pickle")
d = nx_to_graph_data_obj(hy_graph)
com = get_data(d)
torch.save(com,'../data/dataset/processed/dataset_9.pt')
