...
class GCNConv(MessagePassing):
    ...
    def __init__(self, emb_dim, aggr = "add", input_layer = False):
        ...
        self.linear_1 = torch.nn.Linear(1, emb_dim)
        ### Mapping 0/1 edge features to embedding
        self.edge_encoder = torch.nn.Linear(4, emb_dim)
    ...
    def forward(self, x, edge_index):
        ...
        # ad_s = edge_index.size()[1]
        # add features corresponding to self-loop edges.
        # self_loop_attr = torch.zeros(x.size(0), 4)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self_loop_attr = torch.zeros(edge_index[0].size()[1], 4)
        # print("edge_index[0].size()[1]:",edge_index[0].size()[1])
        # print("self_loop_attr:",self_loop_attr.size())
        self_loop_attr[:,1] = 1 # attribute for self-loop edge
        # self_loop_attr = self_loop_attr.to(edge_attr.device).to(edge_attr.dtype)
        # edge_attr = torch.cat((edge_attr, self_loop_attr), dim = 0)
        edge_attr = self_loop_attr.cuda()
        # print("edge_attr:", edge_attr)
        # print("edge_index:", edge_index)
        # println()
        edge_embeddings = self.edge_encoder(edge_attr.float())
        if self.input_layer:
            x = self.input_node_embeddings(x.float())

        norm = self.norm(edge_index[0].long(), x.size(0), x.dtype)
        x = self.linear(x)
        # norm = self.linear_1(norm.view(-1, 1))
        # print("edge_index[0]:", edge_index[0].size(), x.size(), edge_embeddings.size(), norm.size())
    ...
    def message(self, x_j, edge_attr, norm):
        ...
        # print("***:", tmp.size())
        # print("norm:", norm.size())
        # print("&&:", norm.view(-1, 1)*(x_j + edge_attr))
        # println()
...
class GNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        for layer in range(self.num_layer):
            # h = self.gnns[layer](h_list[layer], edge_index, edge_attr)+ 1e-6
            h = self.gnns[layer](h_list[layer], edge_index)+ 1e-6
            # print("---stop here---")
