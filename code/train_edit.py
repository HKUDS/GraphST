...
def train(model: Model, x, edge_index, eps, model_1, optimizer_1,lamb, alpha, beta, steps, node_ratio):
    optimizer.zero_grad()
    ...
    # print("x_1", x_1)
    # println()
    edge_pos_pred = model_1.forward_decoder(x,edge_index)
    ...
    # print("loss_view:",loss_view.mean())
    # pritnln()
    ...
    if eps > 0:  
        ...
        loss2 = loss2.mean()
        loss = (loss1 + eps*loss2+loss_view.mean())
    else: 
        loss = loss1+loss_view.mean()
        loss2 = loss1

    
    loss.backward(retain_graph=True)
    loss_vage = (reward*loss).mean()
    # print("loss_vage:",loss_vage)
    # println()
...
def test(model: Model, x, edge_index, model_1,y, final=False, task ="node"):   
    ...
    file=open(r"./data/tmp_vector_vgae_2.pickle","wb")
...
if __name__ == '__main__':
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        loss1, loss2, loss3 = train(model, x, edge_index, eps, model_generative_1,optimizer_generative_1, lamb, alpha, beta, 5, 0.2)
    ...
    print("=== Final ===")
    results,results_1 = test(model, data.x, data.edge_index, model_generative_1,data.y, final=True)
